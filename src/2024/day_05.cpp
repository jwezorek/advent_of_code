...
namespace {
    ...
    rules rules_to_graph(const std::vector<rule>& edges) {
        rules g;
    ...
    std::tuple<rules, std::vector<update>> parse_input(const std::vector<std::string>& inp) {
    ...
    bool is_valid_update(const rules& g, const update& update) {
    ...
    int do_part_1(const rules& g, const std::vector<update>& updates) {
        return r::fold_left(
    ...
    int do_part_2(const rules& g, const std::vector<update>& updates) {
        return r::fold_left(
            updates | rv::transform(
                [&](auto rule) {
...
void aoc::y2024::day_05(const std::string& title) {
    ...
    std::println("  part 1: {}", do_part_1(rules, updates));
    std::println("  part 2: {}", do_part_2(rules, updates));
